...
const mutations = {
    'UPDATE_CART' (state, {item, quantity, isAdd}) {
        //pronadji element iz liste iz korpe i dodijeli mu id
        const record = state.cartItemList.find(element => element.id == item.id);
        if(record) {
            if(isAdd) {
                //ako dodajem u korpu, povecaj kolicinu proizvoda jednog id-a 
                record.quantity += quantity;
            } else {
                //ako ne dodajem u korpu, neka kolicina proizvoda buste ista
                record.quantity = quantity;
            }
        } else {
            //dodaj u korpu novi element sa novim id-jem
    ...
    'REMOVE_CART_ITEM' (state, {item}) {
        const record = state.cartItemList.find(element => element.id == item.id);
        //ukloni element odredjenog id-a iz korpe 
