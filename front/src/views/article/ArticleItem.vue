...
<div class="mb-3 col-sm-6 col-md-4 item" :class="{'list-group-item': displayList}">
  <div class="thumbnail card">
    <div class="img-event intrinsic">
      <img :src="item.path" alt="" class="grow thumbnail-image card-img-top intrinsic-item p-3">
    </div>
    <div class="card-body">
      <router-link :to="'/articleDetails/' + item.id" tag="h5" class="card-title"><a>{{ item.name }}</a></router-link>
      <p class="card-text truncate">{{ item.description | shortDescription}}</p>
      <div class="row">
        <p class="col-6 lead">${{ item.price }}</p>
      </div>
         <p class="col-6">
          <button class="btn btn-success pull-right" @click="addToCart()">
            Add to cart
          </button>
         </p>
    </div>
